...
#else
#define debug_assert(expr) ((void)((true ? 0 : ((expr)))))
#endif
...
bf_clean(struct bf * const bf);
  ...
  extern const void *
bf_bitmap(const struct bf * const bf);

  extern u64
bf_header_size();

  extern u64
bf_ser_byte(const struct bf * const bf);
...
extern struct rgen * rgen_new_geo(const double p);
