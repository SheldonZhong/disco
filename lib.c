...
static int perf_ctl_fd = 0;
static int perf_ctl_ack_fd = 0;
...
debug_perf_init(void)
{
  ...
  char * const perf_record = strstr(tmp, "perf record");
  char * const perf_stat = strstr(tmp, "perf stat");
  if (perf_record == NULL && perf_stat == NULL)
    return;

  if (perf_record) {
    fprintf(stderr, "%s: perf record detected\n", __func__);
    perf_pid = ppid;
  }

  if (perf_stat)
    fprintf(stderr, "%s: perf stat detected\n", __func__);

  const char * const ctl_fd = getenv("PERF_CTL_FD");
  if (ctl_fd) {
    perf_ctl_fd = atoi(ctl_fd);
    if (perf_ctl_fd)
      fprintf(stderr, "%s: reading PERF_CTL_FD and get fd: %d\n", __func__, perf_ctl_fd);

    const char * const ctl_ack_fd = getenv("PERF_CTL_ACK_FD");
    perf_ctl_ack_fd = ctl_ack_fd ? atoi(ctl_ack_fd) : 0;
    if (perf_ctl_ack_fd)
      fprintf(stderr, "%s: reading PERF_CTL_ACK_FD and get fd: %d\n", __func__, perf_ctl_ack_fd);
  }
}
...
ctl_fifo_init(void)
{
  const char * const ctl_fifo = getenv("PERF_CTL_FIFO");
  if (ctl_fifo) {
    fprintf(stderr, "%s opening PERF_CTL_FIFO %s\n", __func__, ctl_fifo);
    perf_ctl_fd = open(ctl_fifo, O_WRONLY);
    if (perf_ctl_fd)
      fprintf(stderr, "%s: reading PERF_CTL_FIFO and get fifo: %s\n", __func__, ctl_fifo);

    const char * const ctl_ack_fifo = getenv("PERF_CTL_ACK_FIFO");
    fprintf(stderr, "%s opening PERF_CTL_ACK_FIFO %s\n", __func__, ctl_ack_fifo);
    perf_ctl_ack_fd = ctl_ack_fifo ? open(ctl_ack_fifo, O_RDONLY) : 0;
    if (perf_ctl_ack_fd)
      fprintf(stderr, "%s: reading PERF_CTL_ACK_FIFO and get fifo: %s\n", __func__, ctl_ack_fifo);
  }
}
  ...
  void
debug_perf_enable(void)
{
  // blocking to wait for perf to start
  if (perf_ctl_fd == 0)
    return;
  fprintf(stderr, "%s sending enable command\n", __func__);
  dprintf(perf_ctl_fd, "enable\n");

  if (perf_ctl_ack_fd == 0)
    return;

  fprintf(stderr, "%s waiting for ack\n", __func__);
  char ack_buff[5] = {};
  int byte_read = read(perf_ctl_ack_fd, ack_buff, 4);
  if (byte_read != 4 || strcmp(ack_buff, "ack\n") != 0)
    fprintf(stderr, "%s ack received %s != ack\n", __func__, ack_buff);
}

  void
debug_perf_disable(void)
{
  if (perf_ctl_fd)
    dprintf(perf_ctl_fd, "disable\n");
}
...
bf_inc(const u64 hash)
{
  ...
  void
bf_add(struct bf * const bf, const u64 hash64)
{
  u64 t = hash64;
  ...
  bool
bf_test(const struct bf * const bf, const u64 hash64)
{
  u64 t = hash64;
  const u64 inc = bf_inc(hash64);
  const u64 bits = bf->bitmap.nbits;
...
// this is something similar to a static function of bf
  u64
bf_header_size()
{
  ...
  u64
bf_ser_byte(const struct bf * bf)
{
  return bf_header_size() + bf->bitmap.nbytes;
}
...
arena_rank(const size_t sz)
{
  ...
  static bool
arena_slab_init(struct slab * const slab, const u64 obj_size, const u64 blk_size)
{
  const u64 obj0_offset = slab_check_sizes(obj_size, blk_size);
  if (!obj0_offset)
    return false;
...
arena_create(const u64 nr_ranks, const u64 blk_size)
{
  ...
  for (u64 i = 0; i < nr_ranks; i++)
    arena_slab_init(&a->slabs[i], (i << 3) + 8, blk_size);
  return a;
}
...
#define GEN_GEO     ((18))      // Geometric distribution
...
rgen_new_expo(const double percentile, const double range)
{
  ...
  static u64
gen_geo(struct rgen * const gi)
{
  const double u = random_double();
  u64 k;

  const double p = gi->geo.p;
  if (p == 1) {
    k = 0;
  } else {
    k = log (u) / log (1 - p);
...
rgen_helper(const int argc, char ** const argv, struct rgen ** const gen_out)
{
  ...
  if ((0 == strcmp(argv[1], "const")) && (argc >= 3)) {
    ...
    *gen_out = rgen_new_expo(atof(argv[2]), atof(argv[3]));
    return 4;
  } else if ((0 == strcmp(argv[1], "geo")) && (argc >= 3)) {
    *gen_out = rgen_new_geo(atof(argv[2]));
    return 3;
